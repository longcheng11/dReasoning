...
object wholeTest {
  ...
  def main(args: Array[String]) {
    ...
    val path = args(1)
    val N = args(2).toInt
    val filePath_demo = path + "Generated_" + N + "/DEMO" + "14Q3-18Q2/"
    val filePath_drug = path + "Generated_" + N + "/DRUG" + "14Q3-18Q2/"
    val filePath_outc = path + "Generated_" + N + "/OUTC" + "14Q3-18Q2/"
    val filePath_reac = path + "Generated_" + N + "/REAC" + "14Q3-18Q2/"
    val filePath_rpsr = path + "Generated_" + N + "/RPSR" + "14Q3-18Q2/"
    ...
    var factsDF = demoDF.select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate", lit(""))
      .limit(0)

    // DEMO
    /**/
    // report_on_ICSRs_to_FDA
    factsDF = demoDF.select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_on_ICSRs_to_FDA"))

    // report_Whether_15_day_Alert_report_followup_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("i_f_code") === "F")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Whether_15_day_Alert_report_followup_to_FDA")))

    // report_Whether_15_day_Alert_report_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("i_f_code") === "F"
        && demoDF.col("rept_cod") === "EXP")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Whether_15_day_Alert_report_to_FDA")))

    // report_Whether_followup_report_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("i_f_code") === "F"
        && demoDF.col("rept_cod") === "EXP")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Whether_followup_report_to_FDA")))

    // report_Whether_initial_report_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("i_f_code") === "I"
        && demoDF.col("rept_cod") === "EXP")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Whether_initial_report_to_FDA")))

    // report_Date_of_adverse_drug_experience_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("event_dt").isNotNull && demoDF.col("event_dt") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Date_of_adverse_drug_experience_to_FDA")))

    // report_Date_of_ICSR_submission_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("fda_dt").isNotNull && demoDF.col("fda_dt") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Date_of_ICSR_submission_to_FDA")))

    // report_MPD_name_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("mfr_sndr").isNotNull && demoDF.col("mfr_sndr") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_MPD_name_to_FDA")))
    ...
    // report_Patient_gender_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("sex").isNotNull && demoDF.col("sex") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Patient_gender_to_FDA")))

    // report_Patient_information_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("sex").isNotNull && demoDF.col("sex") =!= ""
        && demoDF.col("wt").isNotNull && demoDF.col("wt") =!= ""
        && demoDF.col("age").isNotNull && demoDF.col("age") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Patient_information_to_FDA")))

    // report_Patient_weight_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("wt").isNotNull && demoDF.col("wt") =!= "")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Patient_weight_to_FDA")))

    // report_MPD_Report_source_to_FDA
    factsDF = factsDF.union(demoDF
      .where(demoDF.col("occp_cod") === "MD"
        || demoDF.col("occp_cod") === "PH"
        || demoDF.col("occp_cod") === "OT")
      .select(demoDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_MPD_Report_source_to_FDA")))
    /**/

    // DRUG
    /**/
    // report_Suspect_medical_product_name_to_FDA"
    factsDF = factsDF.union(drugDF
      .where(drugDF.col("drugname").isNotNull && drugDF.col("drugname") =!= "")
      .select(drugDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Suspect_medical_product_name_to_FDA")))

    // report_Suspect_medical_product_route_of_administration_used_to_FDA
    factsDF = factsDF.union(drugDF
      .where(drugDF.col("route").isNotNull && drugDF.col("route") =!= "")
      .select(drugDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Suspect_medical_product_route_of_administration_used_to_FDA")))

    // report_Suspect_medical_Lot_number_to_FDA
    factsDF = factsDF.union(drugDF
      .where(drugDF.col("lot_num").isNotNull && drugDF.col("lot_num") =!= "")
      .select(drugDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Suspect_medical_Lot_number_to_FDA")))

    // report_Suspect_medical_product_Expiration_date_to_FDA
    factsDF = factsDF.union(drugDF
      .where(drugDF.col("exp_dt").isNotNull && drugDF.col("exp_dt") =!= "")
      .select(drugDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Suspect_medical_product_Expiration_date_to_FDA")))

    // report_Suspect_medical_product_NDC_number_to_FDA
    factsDF = factsDF.union(drugDF
      .where(drugDF.col("nda_num").isNotNull && drugDF.col("nda_num") =!= "")
      .select(drugDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_Suspect_medical_product_NDC_number_to_FDA")))
    /**/

    // OUTC
    /**/
    // report_outcome_attributed_to_adverse_drug_experience_to_FDA
    factsDF = factsDF.union(outcDF
      .where(outcDF.col("outc_cod").isNotNull && outcDF.col("outc_cod") =!= "")
      .select(outcDF.col("primaryid").as("argument_X"))
      .withColumn("predicate",lit("report_outcome_attributed_to_adverse_drug_experience_to_FDA")))
    /**/
    ...
    // RPSR
    /**/
    // report_Whether_the_initial_reporter_is_a_health_care_professional_to_FDA
    factsDF = factsDF.union(rpsrDF
      .where(rpsrDF.col("rpsr_cod").isNotNull && rpsrDF.col("rpsr_cod") =!= "")
      .select(rpsrDF.col("primaryid").as("argument_X"))
      .withColumn("predicate", lit("report_Whether_the_initial_reporter_is_a_health_care_professional_to_FDA")))
    /**/
    ...
    */
    ...
    println("WHOLE Total execution time = " + durationSeconds + " seconds")
